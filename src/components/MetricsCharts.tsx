...
// Optimized: Memoized custom tooltip to prevent re-renders
...
const PRVelocityChart = memo(({ prMetrics }: { prMetrics: PRMetric[] }) => (
...
PRVelocityChart.displayName = 'PRVelocityChart';

const BuildPerformanceChart = memo(({ buildMetrics }: { buildMetrics: BuildMetric[] }) => (
  <Card className="glass-card rounded-3xl overflow-hidden border-white/10 group hover:shadow-2xl transition-all duration-500">
...
BuildPerformanceChart.displayName = 'BuildPerformanceChart';

const CodeChurnChart = memo(({ codeChurnMetrics }: { codeChurnMetrics: CodeChurnMetric[] }) => (
  <Card className="glass-card rounded-3xl overflow-hidden border-white/10 group hover:shadow-2xl transition-all duration-500">
...
CodeChurnChart.displayName = 'CodeChurnChart';

const TeamPerformanceTable = memo(({ developerStats }: { developerStats: DeveloperStats[] }) => (
...
TeamPerformanceTable.displayName = 'TeamPerformanceTable';

const TeamDistributionChart = memo(() => (
  <Card className="glass-card rounded-3xl overflow-hidden border-white/10">
...
TeamDistributionChart.displayName = 'TeamDistributionChart';

// Main component with optimizations
const MetricsCharts = memo(function MetricsCharts({ 
  prMetrics, 
  buildMetrics, 
  codeChurnMetrics, 
  developerStats, 
  ...
  const showTeamPerformance = useMemo(() => 
    role === 'team-lead' || role === 'manager', 
    [role]
  );
  
  const showTeamDistribution = useMemo(() => 
    role === 'manager', 
    [role]
  );
  ...
  return (
    <div className="space-y-8">
      <PRVelocityChart prMetrics={prMetrics} />
      <BuildPerformanceChart buildMetrics={buildMetrics} />
      <CodeChurnChart codeChurnMetrics={codeChurnMetrics} />
      
      {showTeamPerformance && (
        <TeamPerformanceTable developerStats={developerStats} />
      )}
      
      {showTeamDistribution && <TeamDistributionChart />}
    </div>
  );
});
